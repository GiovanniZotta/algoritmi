...
string read_output_file = "out/f4/9501293.out";
// string read_output_file = "out/a/96.out";
string folder_name = "f4";
const int CYCLES = 10;
...
void read_output(){
    ...
    for(int i=0; i<devs.size(); i++){
        ...
        if(y == "X"){
            ...
            in >> x;
            // cout << x << " " << y << endl;
            devs[i].pos = {stoi(x) + 1, stoi(y) + 1};
            used_devs[i] = true;
            room[stoi(x) + 1][stoi(y) + 1].second = i;
        }
    }
...
void component_dfs(Component &component, vector<vector<bool>> &visited, ii cell){
    ...
    component.nodes.push_back(cell);
...
Component get_component(vector<vector<bool>> &visited, ii cell){
    Component component;
...
ii find_central_cell(Component &component){
    ii result = NA_PAIR;
    double minimum_distance = DBL_MAX;
    for(ii &cell : component.nodes){
        double average_distance = get_total_distance(component, cell) / component.nodes.size();
        if(average_distance < minimum_distance){
            minimum_distance = average_distance;
            result = cell;
        }
    }
    component.average_distance = minimum_distance;
    return result;
}
...
void compute_graph_sol(){
    ...
    for(Component &component : components){
        if(component.nodes.size() > 1){
            ...
            component.central_cell = cell;
        }
    }
    sort(components.begin(), components.end(), []
        (Component comp1, Component comp2){
            return (double) comp1.average_distance / comp1.nodes.size() < (double) (comp2.average_distance / comp2.nodes.size());
        }
    );
    for(Component &component : components){
        // cerr << "comp average distance: " << component.average_distance << ", comp size: " << component.nodes.size() << ", score: " << component.average_distance / component.nodes.size() << endl;
        // cerr << "central cell: " << component.central_cell.first << " " << component.central_cell.second << endl;
        if(component.nodes.size() > 1){
            // cerr << "central cell : " << "(" << cell.first << ", " << cell.second << ")" << endl;
            start_bfs(component.central_cell);
        }
    }
}
