...
void euler(vector<vector<int>> &graph, vector<int> &euler_tour, vector<int> &first_encounter, vector<int> &new_to_old, int source, int parent){
...
vector<int> distance_from_root(vector<vector<int>> &graph, int root){
    queue<int> q;
...
struct Tree{
    ...
    Tree(int size, int n, int r): tree(size) {
        first_encounter.resize(size);
        cc = n;
        root = r;
    }
...
void solve_graph(vector<vector<int>> &graph, int q){
    ...
    for(int i=0; i<q; i++){
        ...
        if(are_adj(graph, s, t))
            ...
            out << compute_distance(graph, bool_points, s, adj_target) << endl;
        }
    }
}
...
int query(vector<vector<int>> &graph, pair<vector<int>, vector<Tree>> &trees, pair<vector<int>, vector<Clique>> &cliques, vector<bool> &bool_gateways, vector<vector<int>> &tree_gateways, vector<vector<int>> &clique_gateways, int a, int b){
    ...
    q.push(a);
    // cout << "a: " << a << ", b:" << b << endl;
    // cout << "tree[a]: " << trees.first[a] << ", tree[b]:" << trees.first[b] << endl;
    // cout << "clique[a]: " << cliques.first[a] << ", clique[b]:" << cliques.first[b] << endl;
    while(!q.empty()){
        int current = q.top(); q.pop();
        //if it's the target
        // cout << "@@@@@@@@" << endl;
        // cout << "current: " << current << endl;
        // cout << "tree[current]: " << trees.first[current] << ", tree[b]:" << trees.first[b] << endl;
        // cout << "clique[current]: " << cliques.first[current] << ", clique[b]:" << cliques.first[b] << endl;
        if(current == b)
        ...
        if(trees.first[b] == trees.first[current] && trees.first[b] != IMP){
            ...
            if(cliques.first[current] != IMP){
                for(int g : clique_gateways[cliques.first[current]]){
                    if(!visited[g]){
                        // cout << "gateway: " << g << endl;
                        // cout << "tree[gateway]: " << trees.first[g] << endl;
...
void solve(vector<vector<int>> &graph, unordered_map<int, unordered_map<int, bool>> &bool_bridges, int q){
    ...
    // cout << "there are " << *max_element(trees.first.begin(), trees.first.end()) << " trees" << endl;
    ...
    // for(vector<int> g : tree_gateways){
    //     cout << g.size() << " ";
    // }
    ...
    for(int i=0; i<q; i++){
        in >> a >> b;
        out << query(graph, trees, cliques, bool_gateways, tree_gateways, clique_gateways, a, b) << endl;
    }

    //WORKING QUERY
    // cout << trees.second[1].get_distance(10, 6) << endl;
