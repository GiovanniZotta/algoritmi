...
void constructTree(int u, int parent) {
    ...
    for (int v : graph[u]) {
        if (d[v] == -1) {  // not visited
            ...
            if (d[u] <=
                low[v]) {  // u articulation point, I have "closed" a cycle
                int w;
                do {
                    w = s.top();
                    s.pop();
...
void preprocess(int u, int parent) {
    ...
    for (int i = 1; i < LOG; i++)
        if (lca[u][i - 1] != -1)
            lca[u][i] =
                lca[lca[u][i - 1]][i - 1];  // 2^j th ancestor of node i =
                                            // 2^(j-1) th ancestor of 2^(j-1) th
                                            // ancestor of node i

    // compute all ancestors of children
    for (int v : tree[u])
        if (v != parent) preprocess(v, u);
}
...
int getLca(int u, int v) {
    ...
    for (int j = LOG - 1; j >= 0; j--) {
        /*
        We don't need the check lca[u][j] != -1. Indeed, if lca[u][j] = -1, then
        u is the root and since in the for loop u and v are always at the same
        height, this means that u = v = root. But this is impossibile since we
        check lca[u][j] != lca[v][j], before the assignement.
        */
        if (lca[u][j] != lca[v][j]) {
            u = lca[u][j];
            v = lca[v][j];
...
int solve(int u, int v) { return (h[u] + h[v] - 2 * h[getLca(u, v)]) / 2; }
