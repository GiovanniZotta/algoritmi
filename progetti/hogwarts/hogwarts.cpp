...
vector<vector<int>> dijkstra(vector<vector<il>> &graph, int source){
    ...
    distance[source] = 0;
    vll s = {source, 0, NONE}; // 0 -> current node, 1 -> cost, 2 -> parent
    q.push(s);
        ...
        if(!visited[node[CURRENT]]){
            ...
            for(il &adj : graph[node[CURRENT]])
                if(!visited[adj.first]){
                    ...
                    if(new_dist <= distance[adj.first]){
...
int solve(vector<vector<int>> &dag, stack<int> &top_sort_stack){
    ...
    return *max_element(dominated_nodes.begin(), dominated_nodes.end());
}
...
int main(){
    ...
    cout << solve(dag, top_sort_stack) << endl;
    return 0;
